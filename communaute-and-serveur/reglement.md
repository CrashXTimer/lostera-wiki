# Règlement du serveur

{% hint style="danger" %}
Nous ne sommes pas responsables si vous ne connaissez pas les règles ! Il est de la responsabilité de chaque joueur de les connaître et de se tenir informé des changements éventuels.
{% endhint %}

En rejoignant LostEra, tu acceptes l'intégralité de ce règlement, ainsi que le [Règlement Chat](https://docs.google.com/document/u/0/d/1bOoPhP-Lg5Oc_5V44zJBsYt81R1KHQ10ve-IEUdbIeA/edit) et le [Règlement Mode GENS](https://docs.google.com/document/u/0/d/1gjMvEqDgJf0KJ0RXG7vP6W76hPWaC2rSPiz5CAWsK7A/edit), qui complètent ce document.

## Réinitialisation des sanctions

Tous les avertissements et les mutes sont réinitialisés après **7 jours**. Si tu as été averti dans les 7 derniers jours, la chaîne de sanctions se poursuit ; passé ce délai, elle repart à zéro. Toute tentative d'abus de ce système entraîne une sanction de la part de l'équipe.

## Informations importantes

* Les règles doivent être suivies exactement : aucun membre du staff ne peut les modifier à sa convenance, sauf lorsqu'une sanction n'est pas explicitement prévue mais s'impose clairement.
* Contourner une règle par plaisanterie entraîne une sanction définitive ; faire un appel par plaisanterie peut entraîner une sanction plus sévère encore.
* « Gestion » désigne tout membre du staff ayant le rang d'administrateur ou plus. Les décisions de gestion sont prises collégialement, jamais par un seul membre.
* Pour un litige avec un membre ayant le rang d'administrateur ou plus, contacte directement **pxnshy** ou **xmrmitsuki** sur Discord (pas de ticket).
* Pour proposer une nouvelle règle, contacte **pxnshy** ou **xmrmitsuki** sur Discord. Pour clarifier une règle existante, ouvre un ticket Discord.
* Une sanction n'est annulée que si elle a été appliquée à tort.

## Directives diverses

* Pour signaler un membre du staff, ouvre un ticket Discord sans mentionner son nom tant qu'il est encore en poste.
* Créer de faux tickets (ex. fausse sanction injuste) peut entraîner un bannissement permanent. La sévérité reste à la discrétion d'un administrateur ou plus.
* Un appel de bannissement rejeté peut faire l'objet d'une demande de clarification ; si les preuves sont évidentes, l'appel est clos.
* En cas de mort causée par un avantage déloyal ou un hack, ouvre un ticket avec preuves.
* Pour signaler une île utilisant des exploits, des preuves plausibles sont requises.
* Le Discord doit rester un espace décontracté ; en cas de sanction jugée injuste, contacte un supérieur.

## Menaces de DDOS, DOX et SWAT

Non tolérées, y compris sous forme de blague — une blague confirmée peut réduire la sanction à un bannissement de 14 jours.

**1ère infraction : Blacklist permanente.**

## Échange en réel (IRL Trading)

Toute preuve suffisante d'un échange en argent réel (ex. PayPal) entraîne une sanction.

| Infraction | Sanction |
|---|---|
| 1ère | Bannissement de 14 jours |
| 2ème | Bannissement permanent |

## Intention de jeu

Provoquer des crashes, créer des ralentissements significatifs, perturber l'équilibre du serveur, divulguer des informations confidentielles sur des mises à jour/événements, ou utiliser des exploits non prévus : sanctionné selon la gravité, à la décision d'un administrateur ou plus.

## Escroquerie de crédits

Strictement interdite, que ce soit pour des crédits ou des objets — l'escroc perd tous les objets échangés.

En cas de chargeback (un joueur achète des crédits via le pseudo d'un autre sur Tebex pour un échange en jeu, puis conteste le paiement) : la victime ne paie pas à nouveau mais perd l'objet/rang échangé, l'auteur du litige est banni définitivement, et le joueur lésé récupère ses objets.

| Infraction | Sanction |
|---|---|
| 1ère | Bannissement de 30 jours |
| 2ème | Bannissement permanent |

## Litiges et chargebacks (Tebex / paiement)

{% hint style="danger" %}
Tous les paiements sont **définitifs et non remboursables**. Toute tentative de rétrofacturation ou l'ouverture d'un litige (PayPal ou autre) entraîne un **bannissement permanent et irréversible** de tous nos serveurs. Le droit de rétractation légal ne s'applique pas à ce contenu numérique non fourni sur support matériel.
{% endhint %}

## Usurpation d'un membre du staff

Se faire passer pour un membre du staff (dans le chat ou en le prétendant) est interdit.

| Infraction | Sanction |
|---|---|
| 1ère | Mute de 6 heures |
| 2ème | Bannissement d'un jour |

---

## Règlement Chat

Le [Règlement Chat](https://docs.google.com/document/u/0/d/1bOoPhP-Lg5Oc_5V44zJBsYt81R1KHQ10ve-IEUdbIeA/edit) détaille les règles spécifiques au chat, réparties dans les catégories suivantes :

* Discrimination
* Racisme / Intolérance / Harcèlement / Termes Dérogatoires / Sujets Sensibles
* Publicité
* Menaces de Mort / Encouragement au Suicide
* Spam dans le Chat, Spam de Caractères, Flood Chat, Encouragement au Spam et Majuscules Excessives
* Mute évasion
* Toxicité
* Juron
* Autres Langues dans le Chat
* Menaces de DDOS, DOX et SWAT

{% hint style="info" %}
Le détail des sanctions de chaque catégorie est disponible directement dans le document Google Docs lié ci-dessus.
{% endhint %}

## Règlement Mode GENS

Le [Règlement Mode GENS](https://docs.google.com/document/u/0/d/1gjMvEqDgJf0KJ0RXG7vP6W76hPWaC2rSPiz5CAWsK7A/edit) couvre les règles propres à la progression et aux îles :

* Intrusion sur l'île
* Fusion d'îles
* Îles Alternatives
* Échanges pour Valeur d'île
* Règle de disqualification
* Boosting de l'île / Boosting pour les Payouts
* Avantage Injuste
* Farming Automatisé
* Exploitation, Duplication ou Glitch
* Botting
* Partage de Compte
* Arnaque
* Comptes Alternatifs (Alts)
* Nom d'île inapproprié

{% hint style="info" %}
Le détail des sanctions de chaque catégorie est disponible directement dans le document Google Docs lié ci-dessus.
{% endhint %}
